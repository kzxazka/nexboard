...
const executeDelete = () => {
    if (itemToDelete.value) {
        router.delete(route('sketches.destroy', itemToDelete.value), {
            onSuccess: () => {
    ...
    <AuthenticatedLayout>
        ...
        <div class="page-transition">
            <div v-if="sketches.length === 0" class="glass-card p-12 text-center">
                <h3 class="text-lg font-semibold text-nexboard-on-surface mb-1">No sketches yet</h3>
                ...
                <div v-for="(sketch, i) in sketches" :key="sketch.id" class="glass-card overflow-hidden hover:border-indigo-500/30 group animate-slide-up" :style="{ animationDelay: `${i * 50}ms` }">
                    ...
                    <div class="p-4 flex items-start justify-between">
                        <div class="min-w-0 flex-1">
                            <Link :href="route('sketches.show', sketch.id)" class="text-sm font-semibold text-nexboard-on-surface truncate block group-hover:text-indigo-400 transition-colors">{{ sketch.title }}</Link>
                            <p class="text-xs text-nexboard-on-surface-variant mt-1">{{ sketch.project?.name }} · {{ formatDate(sketch.created_at) }}</p>
                        </div>
                        <button @click.prevent="confirmDelete(sketch.id)" class="opacity-0 group-hover:opacity-100 p-1 rounded hover:bg-red-500/20 transition-all shrink-0 ml-2">
            ...
            <Transition enter-active-class="transition-all duration-300" leave-active-class="transition-all duration-200" enter-from-class="opacity-0" leave-to-class="opacity-0">
                <div v-if="showCreateModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4">
                    ...
                    <div class="glass-card-elevated w-full max-w-md p-6 relative animate-slide-up z-10">
                        <h2 class="text-lg font-semibold text-nexboard-on-surface mb-4">New Sketch</h2>
                        <form @submit.prevent="submit" class="space-y-4">
                            <div>
                                ...
                                <select v-model="form.project_id" class="input-dark" required>
                                    <option value="" disabled class="bg-nexboard-surface-container-high text-slate-400">Select a project</option>
                                    <option v-for="p in projects" :key="p.id" :value="p.id" class="bg-nexboard-surface-container-high text-nexboard-on-surface">{{ p.name }}</option>
                                </select>
                            </div>
                            <div>
